...
- **Trained and available locally**, but this file is gitignored
  (`ml/models/*.joblib` in `.gitignore`) and is therefore **not** in the
  repo — each teammate must generate their own copy by running:
python ml/train.py
  This regenerates the synthetic training data (if missing) and trains
  both the Scholarship CART model and this Random Forest model, saving
  both to `ml/models/`. Takes a few seconds.
- Current validated performance (synthetic data, 5-fold cross-validation):
  test accuracy 0.82, precision 0.796, recall 0.830, F1 0.8125,
  cv5-mean 0.8083 (std 0.012). Full metrics: `ml/models/metrics.json`
  (also gitignored — regenerated by the same `train.py` run).
- If `ml/models/placement_rf.joblib` is absent (e.g. before your first
  `python ml/train.py` run), the agent still works correctly, falling back
  to rules-only evaluation: every shortlist entry says `"Meets all drive
  criteria (model unavailable, rules-only evaluation)"` and
  `model_version: null`. This is expected on a fresh checkout, not a bug —
  just run `python ml/train.py` once to enable ML scoring.
