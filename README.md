...
| Librarian | `librarian1` / `librarian123` | catalogue management (add/edit/remove books, stock) and institution-wide oversight of every reservation, issue, fine and review — the Library Agent runs requests/issuing/fines automatically, the librarian doesn't approve individual reservations |
...
Finance, Placement, Notification, and (added post-P2) Library stay in the
registry — tools.py and the REST routes call into them exactly as before —
but fail the criterion above: Academic/Admission/Finance/Placement/Library
have no policy of their own that outlives a request beyond simple
config-driven rules, and Notification reacts to events but owns neither
state nor policy, so it is a bus subscriber, not an agent.
...
*(Library was reintroduced after P2 as a registry module — catalogue,
reservations, issuing, fines, recommendations — with its own automated
pickup-deadline sweep, same non-core treatment as Finance/Placement above.
It still fails the core-agent criterion: no policy beyond fixed,
config-driven rules, so `CORE_AGENTS` is unchanged.)*
...
backend/app/
  ...
  scheduler_live.py  additive event-trace wrapper for the live simulation UI (not research)
    ...
    library.py         catalogue, requests, issue/return, fines, recommendations,
                       proactive pickup-deadline sweep [tool-backed, not an agent]
  seed_library.py    synthetic ~500-book catalogue + borrowing history (kept
                     separate from seed.py, which stays untouched)
