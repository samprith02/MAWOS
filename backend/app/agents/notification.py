...
class NotificationAgent(BaseAgent):
    ...
    def register_subscriptions(self):
        ...
        self.bus.subscribe("library.request_expired", self.name, self.on_library_request_expired)
        self.bus.subscribe("library.book_returned", self.name, self.on_library_book_returned)
    ...
    async def on_library_request_expired(self, payload: dict):
        db = self.session()
        try:
            self._notify(db, "Library pickup window expired",
                         f"{payload.get('count', 0)} book reservation(s) expired "
                         f"unpicked-up today; a flat fine was applied to each.",
                         role="admin")
            db.commit()
        finally:
            db.close()

    async def on_library_book_returned(self, payload: dict):
        if not payload.get("overdue_days"):
            return
        db = self.session()
        try:
            self._notify(db, "Library fine — late return",
                         f"Your book was returned {payload['overdue_days']} day(s) "
                         f"late. A fine of Rs {payload.get('fine_amount', 0)} has "
                         f"been added to your account.", usn=payload["usn"])
            db.commit()
        finally:
            db.close()
