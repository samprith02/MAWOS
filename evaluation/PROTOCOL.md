...
| 2026-08-31 | **v4 R0.5 executed — provider viability gate. No candidate eligible; no provider adopted.** New instrument `evaluation/provider_probe.py` + `evaluation/probe/` (25 frozen items, fingerprint `9842bb29d6c5b63e…`, asserted disjoint from `DEV_TASKS` — the assertion fired once and the offending item was reworded, not the check weakened). Nine measures with thresholds pre-registered in `docs/v4/03_LLM_LAYER.md` §4.3 **before any provider was run**, and applied unchanged. Results (3 seeds, temperature 0, 225 item-runs, 390 LLM calls): qwen2.5:1.5b-instruct fails 7/8, 3b fails 5/8, 7b fails 4/8; selection scores 12.5% / 37.5% / 75.0%. The two hosted candidates (`gemini-2.0-flash`, `groq:llama-3.3-70b`) were **not testable** — no credential in the environment — and are recorded as untested with no score estimated. 7B was 81.7% GPU-resident and is out-of-competition per §9.2, the same figure and the same treatment as the pre-P2 and P6 sweeps. Shipped `mawos.db` untouched (probe ran against a scratchpad copy; sha256 verified identical before and after). | Per §4.3's own clause and `OPEN_DECISIONS.md` D1, **thresholds are not relaxed when nothing passes** — the null outcome is the finding. D1 stays OPEN and R3 is blocked on either testing a hosted provider or a recorded re-registration. Two limitations found during the run are logged in `evaluation/results/v4_gates/r05_findings.md` rather than silently corrected: (a) M7's 3 s "single-tool turn" threshold is unachievable by construction here, because a single-tool turn costs **two** LLM calls (measured: 100% of wall time is LLM time) — flagged for re-registration *before* any re-run, not adjusted now; (b) a reproducible false positive in `backend/app/provenance.py` (`_NUMBER_RE`'s leading `\s*` breaks the line-start maths that the markdown-list-marker filter depends on), which depresses M6 — recorded against `OPEN_DECISIONS.md` D10 and **not fixed**, since `provenance.py` is under `FROZEN.sha256` discipline and R0.5 is a provider gate, not a gate-repair phase. No existing clause changed. |
| 2026-09-01 | **M7 re-registered** (v4 R0.5 instrument): per-LLM-call p50 <= 3.0 s, single-tool turn <= **6.0 s**, 3-step plan <= 15.0 s, replacing the original "turn <= 3.0 s". A per-call diagnostic (M7b) is added. `docs/v4/03_LLM_LAYER.md` gains §2.3.1; `OPEN_DECISIONS.md` D13 closed. `provider_probe.report()` gains an instrument-version guard that refuses to overwrite a result file scored against a different threshold set, writing to a dated filename instead — verified firing. | **This is a post-hoc threshold change, which §1 rule 8 normally forbids**, and it is admitted only because all four bounding constraints hold and were checked *before* the change: (1) it rescues nothing — with M7 deleted outright the 2026-08-31 eligible set is still empty (1.5B fails 6 other mandatory measures, 3B 4, 7B 3), and at 6.0 s all three still fail M7 (8.12/9.40/11.22 s); (2) the 2026-08-31 run is **not** re-scored and `r05_provider.{json,md}` stand as produced at 3.0 s; (3) the new value applies only to future runs, which are a **new instrument** whose numbers may never be differenced against 2026-08-31; (4) the motivation is structural, not empirical — the original threshold contradicted `01_ARCHITECTURE.md` §6's own definition of a turn as two LLM calls, so no model quality could have satisfied it. Per-call p50 on 2026-08-31 was 3.79/4.16/5.36 s, above the new 3.0 s budget, so the re-framing does not flatter the local models either. No other clause changed; D1 remains OPEN. |
| 2026-09-01 | **R0.5 hosted run (Gemini). No candidate eligible; D1 still OPEN.** `gemini-3.5-flash-lite` completed the full 25-item x 3-seed probe (75 records, 144 LLM calls, 0 hard failures) and was scored under **both** threshold sets -- the 2026-08-31 baseline (M7<=3.0 s) and the current one (M7<=6.0 s). Result is the same under both: M1 100%, M2 87.5%, M3 80%, **M4 0% FAIL**, M5 100%, M6 100%, M7 8.11 s wall **FAIL**, M9 0%. The frontier-tier class (`gemini-2.5-flash`) is **untestable on this free tier**: quota is 20 generate-content requests per day per model, against ~150 needed; 14 records captured then exhausted, recorded as an incomplete run that the report refuses to score as eligible. Local results unchanged and not re-scored. Full analysis: `evaluation/results/v4_gates/r05_gemini_findings.md`. | Thresholds were applied exactly as pre-registered and **none was changed after seeing results**. Three methodological facts recorded rather than acted on: (a) **D14 opened** -- M7 is computed from wall-clock time, which includes this harness's own rate pacing for hosted providers and nothing for local ones, so the two classes are not comparable on it (Gemini: 8.11 s wall vs 3.32 s provider-only, 4.79 s of it our own sleep; per-call p50 1.56 s). Left unapplied, partly because the fix would help the strongest candidate, and it changes no outcome since Gemini fails M4 regardless. (b) A Gemini 3.x adapter trap was found and fixed **before** the scored run: thinking models require the `thoughtSignature` issued with a `functionCall` to be echoed on the follow-up turn, and dropping it fails every multi-round item -- an adapter bug that would have been recorded as the model being incapable of multi-step work. (c) The probe's scratch database had drifted behind R1's schema (`subjects.kind`), surfacing a tool exception as a model failure; repaired by a **schema-only** migration that changed no row values, preserving the frozen probe items and their actors. No clause changed. |
